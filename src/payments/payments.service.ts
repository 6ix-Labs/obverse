...
export class PaymentsService {
     ...
     */
    async createPaymentFromFrontend(dto: CreatePaymentDto): Promise<PaymentDocument> {
        // 1. Validate and get the payment link
        const paymentLink = await this.paymentLinksService.findByLinkIdWithMerchant(dto.linkCode);

        // 2. Check if this transaction was already recorded (prevent duplicates)
        const existingPayment = await this.findByTxSignature(dto.txSignature, dto.chain);
        if (existingPayment) {
            this.logger.warn(`Payment with tx ${dto.txSignature} already exists`);
            throw new ConflictException('Payment with this transaction signature already exists');
        }

        // 3. Validate the payment amount matches the link
        ...
        // 6. Determine if payment should be marked as confirmed
        ...
        this.logger.log(`Payment created from frontend: ${payment._id} for link ${dto.linkCode} (status: ${isConfirmed ? 'confirmed' : 'pending'})`);

        // Return the updated payment with confirmed status
        const updatedPayment = await this.paymentModel.findById(payment._id);
        if (!updatedPayment) {
            throw new NotFoundException(`Payment ${payment._id} not found after creation`);
        }
    ...
    async createPayment(data: {
        ...
        try {
            ...
            if (!existingTx) {
                ...
                this.logger.log(`Transaction record created for payment ${savedPayment._id}`);
            } else {
                this.logger.warn(`Transaction ${data.txSignature} already exists, skipping creation`);
            }
        } catch (error) {
            // Log error but don't fail the payment creation
            this.logger.error(`Failed to create transaction record: ${error.message}`, error.stack);
        }

        return savedPayment;
    }

    async findByTxSignature(txSignature: string, chain?: string): Promise<PaymentDocument | null> {
    ...
    async getPaymentStats(merchantId: string, chain?: string): Promise<{
