...
export class CreateLinkHandler {
    ...
    async handle(ctx: any) {
        ...
        if (!merchant || !merchant.walletAddress) {
            await ctx.reply(
                `⚠️ Please set up your wallet first using /start`,
            );
            return;
        }
    ...
    async handleCustomFieldsInput(ctx: any, state: any) {
        ...
        const fieldsText = customFields.length > 0
            ? `• ${customFields.join('\n• ')}`
            : 'None';
    ...
    async handleDescriptionInput(ctx: any, state: any) {
        ...
        const finalDescription = description.toLowerCase() === 'skip' ? '' : description;
                ...
                reply_markup: {
                    inline_keyboard: [
                        [
                            ...
                            { text: '📊 View Stats', callback_data: `view:${paymentLink.linkId}` },
                        ],
