...
export class SettingsHandler {
    ...
    private async showSettings(ctx: any, merchant: any) {
        const notificationStatus = merchant.notificationsEnabled ? '🔔 ON' : '🔕 OFF';
        const webhookStatus = merchant.webhookUrl ? '✅ Set' : '❌ Not set';

        const keyboard = {
            inline_keyboard: [
                [
                    { text: `Default Token: ${merchant.defaultToken}`, callback_data: 'setting:token' },
                ],
                [
                    { text: `Notifications: ${notificationStatus}`, callback_data: 'setting:notifications' },
                ],
                [
                    { text: `Webhook: ${webhookStatus}`, callback_data: 'setting:webhook' },
                ],
                [
    ...
    async updateToken(ctx: any, token: string) {
        ...
        await this.showSettings(ctx, await this.merchantsService.findByTelegramId(telegramId));
    }
    ...
    async startDefaultFieldsSetup(ctx: any) {
        ...
        const keyboard = {
            inline_keyboard: [
                [
                    { text: '📋 Name & Email', callback_data: 'default_fields:name,email' },
                ],
                [
                    { text: '📞 Full Contact', callback_data: 'default_fields:name,email,phone' },
                ],
